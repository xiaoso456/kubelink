...
public class Copy extends Exec {
  ...
  private void createFiles(
    ...
    for (TreeNode childNode : node.getChildren()) {
      if (!childNode.isDir()) {
        ...
        String filePath = genericPathBuilder(destination.toString(), childNode.getName());
        File f = new File(filePath);
        if (!f.createNewFile()) {
          throw new IOException("Failed to create file: " + f);
        }
        String modifiedSrcPath = genericPathBuilder(srcPath, childNode.getName());
        ...
        String newSrcPath = genericPathBuilder(srcPath, childNode.getName());
        // TODO: Change this once the method genericPathBuilder is changed to varargs
        Path newDestination = Paths.get(destination.toString(), childNode.getName());
  ...
  private void createDirectory(TreeNode node, Path destination) throws IOException {
    ...
    for (TreeNode childNode : node.getChildren()) {
      if (childNode.isDir()) {
        String path = genericPathBuilder(destination.toString(), childNode.getName());
      ...
      try (InputStream is = proc.getInputStream();
        ...
        while ((line = reader.readLine()) != null) {
          ...
          if (line.charAt(line.length() - 1) == '/') {
            ...
            String path = genericPathBuilder(srcPath, subDirTree.getName());
  ...
  */
  ...
  private static class ProcessFuture implements Future<Integer> {
    ...
    public Integer get(long timeout, TimeUnit unit) throws InterruptedException {
      boolean done = waitForDone(timeout, unit);
      proc.destroy();
      if(done){
        return 0;
      }
    ...
    private boolean waitForDone(long timeout,TimeUnit unit) throws InterruptedException {
